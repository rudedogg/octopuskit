...
//    }
    ...
    public override func didAddToEntity() {
        ...
        if  !(self.entity?.node is SKScene) {
            OctopusKit.logForWarnings("\(self) added to a child entity instead of the OKScene.entity: \(entity) — Events may not be received!")
            OctopusKit.logForTips ("Use RelayComponent(for:) to add a relay to the scene's sharedPointerEventComponent, or override the scene's input handling methods.")
        }
    }
